    ...
    def __call__(
        ...
        *,
        ...
        if self.dspark and B == 1 and L == 1:
            ...
            out = None
            # [I296] 수치는 무죄([I293])이나 TP2 서빙에서 이 분기 활성 시
            # 첫 요청 행 2/2 재현 → 크래시 3·4차와 상관. 행 기전 미해명 상태로
            # 라이브 재차단(오프라인 정밀 격리 전까지).
            if (os.getenv("OMLX_WSDPA_MTP") == "1"
                    and _standard_mask and B == 1 and L > 1):
                out = wsdpa_prefill(
                    q,
                    kv,
                    None,
