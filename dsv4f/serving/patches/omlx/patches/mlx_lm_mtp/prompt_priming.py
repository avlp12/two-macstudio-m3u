...
class _IntOffsetView:
    """배치 캐시(B==1)의 array-offset을 int로 노출하는 앵커 프록시.

    BatchRotatingKVCache.offset은 mx.array([N])라 plain-int 프로브가 침묵
    탈락(배치 경로 priming 사망의 근본 원인, 2026-08-24 [I289]). (1,S) 입력
    가드가 B==1을 보장하므로 size-1 배열만 int로 풀어준다."""
    __slots__ = ("_c",)
    def __init__(self, c): self._c = c
    @property
    def offset(self):
        o = getattr(self._c, "offset", None)
        if type(o) is int:
            return o
        if getattr(o, "size", 0) == 1:
            try:
                return int(o.reshape(-1)[0])
            except Exception:
                return None
        return None


def _int_offset_ok(c) -> bool:
    o = getattr(c, "offset", None)
    return type(o) is int or getattr(o, "size", 0) == 1
...
def _anchor(cache: Optional[List[Any]]) -> Optional[Any]:
    ...
    for c in cache:
        if _int_offset_ok(c):
            return _IntOffsetView(c)
        for sub in getattr(c, "caches", ()) or ():
            if _int_offset_ok(sub):
                return _IntOffsetView(sub)
    return None
...
def take_primed(
    ...
    for host in _host_candidates(model):
        ...
        if callable(hook):
            r = hook(cache, main_tok)
            if r is not None:
                return r
            # 훅이 사양(None)하면 제네릭 심으로 폴스루 — deepseek_v4의 훅은
            # DSpark 전용이라 legacy MTP에서 무조건 None을 반환해 priming이
            # 구조적으로 죽어 있었다(2026-08-24 [I289]).
            break
    ctx = _find_ctx(model)
    if ctx is None:
        return None
